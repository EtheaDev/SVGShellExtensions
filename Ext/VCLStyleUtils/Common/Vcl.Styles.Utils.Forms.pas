...
//    Include(TextFormat, tfRtlReading);
  ...
  begin
    ...
    if Assigned(Application.Mainform) then
      StyleServices.DrawText(Canvas.Handle, CaptionDetails, LText, TextRect, TextFormat, clRed
        {$IF (CompilerVersion >= 33)}, Application.MainForm.Monitor.PixelsPerInch{$ENDIF})
    else
      StyleServices.DrawText(Canvas.Handle, CaptionDetails, LText, TextRect, TextFormat, clRed
        {$IF (CompilerVersion >= 33)}, Screen.PixelsPerInch{$ENDIF});
    MoveWindowOrg(Canvas.Handle, 0, -TextTopOffset);
  end
  else
  begin
